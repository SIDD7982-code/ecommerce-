...
def topwear(request, data=None):
 if data == None:
  topwears = Product.objects.filter(category='TW')
 elif data == 'peterengland' or data == 'ucb':
...
def product_list_view(request):
    ...
    results = Product.objects.filter(name__icontains=product) if product else Product.objects.all()

    return render(request, 'app/base.html', {'results': results, 'product': product})
...
def search_view(request):
    ...
    if query:
        ...
        products = products.filter(
            ...
            models.Q(discounted_price__icontains=query)
        ).distinct()  # Use distinct to avoid duplicates

    return render(request, 'app/base.html', {'query': query, 'products': products})